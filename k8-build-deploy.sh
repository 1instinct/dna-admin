...
# ============================================================
# dna-admin: Build, Push, and Deploy to MicroK8s cluster
# Builds natively on Pi (no cross-compilation needed).
# Run from the dna-admin directory on the dev machine.
# ============================================================

REGISTRY_HOST="${REGISTRY_HOST:-smokey01}"
REGISTRY_PORT="32000"
IMAGE_NAME="dna-admin"
TAG="${TAG:-latest}"
...
ssh "${KUBECTL_NODE}" sudo microk8s kubectl rollout restart deployment/dna-admin -n default
ssh "${KUBECTL_NODE}" sudo microk8s kubectl rollout status deployment/dna-admin -n default --timeout=120s

echo ""
echo "=== Deploy complete ==="
ssh "${KUBECTL_NODE}" sudo microk8s kubectl get pods -n default -l app=dna-admin -o wide
